...
# Balance Table 
# format table
balance_table <- as.data.frame(cov_balance$results)

kable(balance_table, format = "html")
...
smoking$pscores <- predict(propensity, type = "response")
...
stargazer(propensity_lm, type = "html")
```
...
cov_balance_ps$results

## Histogram of PS after matching
histbackback(split(match.data$pscores, match.data$tobacco),	main = 
...
There's only a slight difference between the estimates of the ATT and the treatment of the ATE
...
fd_manual <- lm(vani_fd ~ treatment)
